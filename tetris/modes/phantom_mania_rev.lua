...
local PhantomManiaGameRev = GameMode:extend()

PhantomManiaGameRev.name = "Phantom Mania Rev"
PhantomManiaGameRev.hash = "PhantomManiaRev"
PhantomManiaGameRev.tagline = "The blocks disappear as soon as they're locked! Can you remember where everything is?"

function PhantomManiaGameRev:new()
	PhantomManiaGameRev.super:new()
...
function PhantomManiaGameRev:getARE()
...
function PhantomManiaGameRev:getLineARE()
...
function PhantomManiaGameRev:getDasLimit()
...
function PhantomManiaGameRev:getLineClearDelay()
	return self:getLineARE() - 2
end

function PhantomManiaGameRev:getLockDelay()
...
function PhantomManiaGameRev:getGravity()
	return 20
end

function PhantomManiaGameRev:hitTorikan(old_level, new_level)
...
function PhantomManiaGameRev:advanceOneFrame()
	if self.clear then
...
function PhantomManiaGameRev:onPieceEnter()
	if (self.level % 100 ~= 99 and self.level ~= 998) and not self.clear and self.frames ~= 0 then
		self.level = self.level + 1
	end
end

function PhantomManiaGameRev:onLineClear(cleared_row_count)
...
function PhantomManiaGameRev:updateScore(level, drop_bonus, cleared_lines)
	if not self.clear then
...
PhantomManiaGameRev.rollOpacityFunction = function(age)
	if age > 4 then return 0
	else return 1 - age / 4 end
end

function PhantomManiaGameRev:drawGrid()
...
function PhantomManiaGameRev:qualifiesForGM()
    return true
end

function PhantomManiaGameRev:drawScoringInfo()
	PhantomManiaGameRev.super.drawScoringInfo(self)
...
function PhantomManiaGameRev:getSectionEndLevel()
	if self.level >= 900 then return 999
	else return math.floor(self.level / 100 + 1) * 100 end
end

function PhantomManiaGameRev:getBackground()
	return math.floor(self.level / 100)
end

function PhantomManiaGameRev:getHighscoreData()
	return {
		level = self.level,
		frames = self.frames,
	}
