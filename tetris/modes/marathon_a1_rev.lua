...
local MarathonA1GameRev = GameMode:extend()

MarathonA1GameRev.name = "Marathon A1 Rev"
MarathonA1GameRev.hash = "MarathonA1Rev"
MarathonA1GameRev.tagline = "Can you score enough points to reach the title of Grand Master?"



function MarathonA1GameRev:new()
	MarathonA1GameRev.super:new()
...
function MarathonA1GameRev:getARE()
	return 30
end

function MarathonA1GameRev:getLineARE()
	return 27
end

function MarathonA1GameRev:getDasLimit()
	return 15
end

function MarathonA1GameRev:getLineClearDelay()
	return 44
end

function MarathonA1GameRev:getLockDelay()
	return 30
end
...
function MarathonA1GameRev:getGravity()
...
function MarathonA1GameRev:advanceOneFrame()
	if self.clear then
...
function MarathonA1GameRev:onPieceEnter()
	if (self.level % 100 ~= 99 and self.level ~= 998) and not self.clear and self.frames ~= 0 then
		self.level = self.level + 1
	end
end

function MarathonA1GameRev:onLineClear(cleared_row_count)
...
function MarathonA1GameRev:updateScore(level, drop_bonus, cleared_lines)
	if not self.clear then
...
function MarathonA1GameRev:checkGMRequirements(old_level, new_level)
...
function MarathonA1GameRev:drawGrid()
	self.grid:draw()
...
function MarathonA1GameRev:drawScoringInfo()
	MarathonA1GameRev.super.drawScoringInfo(self)
...
function MarathonA1GameRev:getSectionEndLevel()
	if self.level >= 900 then return 999
	else return math.floor(self.level / 100 + 1) * 100 end
end

function MarathonA1GameRev:getBackground()
	return math.floor(self.level / 100)
end

function MarathonA1GameRev:getHighscoreData()
	return {
		grade = self.grade,
		score = self.score,
		level = self.level,
