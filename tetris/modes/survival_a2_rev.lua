...
local SurvivalA2GameRev = GameMode:extend()

SurvivalA2GameRev.name = "Survival A2 Rev"
SurvivalA2GameRev.hash = "SurvivalA2Rev"
SurvivalA2GameRev.tagline = "The game starts fast and only gets faster!"

function SurvivalA2GameRev:new()
	SurvivalA2GameRev.super:new()
	self.roll_frames = 0
	self.combo = 1
...
function SurvivalA2GameRev:getARE()
...
function SurvivalA2GameRev:getLineARE()
...
function SurvivalA2GameRev:getDasLimit()
...
function SurvivalA2GameRev:getLineClearDelay()
	return self:getLineARE() - 2
end

function SurvivalA2GameRev:getLockDelay()
...
function SurvivalA2GameRev:getGravity()
	return 20
end

function SurvivalA2GameRev:hitTorikan(old_level, new_level)
...
function SurvivalA2GameRev:advanceOneFrame()
	if self.clear then
...
function SurvivalA2GameRev:onPieceEnter()
	if (self.level % 100 ~= 99 and self.level ~= 998) and not self.clear and self.frames ~= 0 then
		self.level = self.level + 1
	end
end

function SurvivalA2GameRev:onLineClear(cleared_row_count)
...
function SurvivalA2GameRev:updateScore(level, drop_bonus, cleared_lines)
	if not self.clear then
...
function SurvivalA2GameRev:getLetterGrade()
...
function SurvivalA2GameRev:drawGrid()
	self.grid:draw()
end

function SurvivalA2GameRev:drawScoringInfo()
	SurvivalA2GameRev.super.drawScoringInfo(self)
...
function SurvivalA2GameRev:getSectionEndLevel()
	if self.clear then return self.level
	elseif self.level >= 900 then return 999
	else return math.floor(self.level / 100 + 1) * 100 end
end

function SurvivalA2GameRev:getBackground()
	return math.floor(self.level / 100)
end

function SurvivalA2GameRev:getHighscoreData()
	return {
		level = self.level,
		frames = self.frames,
	}
