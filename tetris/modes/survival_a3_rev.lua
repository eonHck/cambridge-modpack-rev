...
local SurvivalA3GameRev = GameMode:extend()

SurvivalA3GameRev.name = "Survival A3 Rev"
SurvivalA3GameRev.hash = "SurvivalA3Rev"
SurvivalA3GameRev.tagline = "The blocks turn black and white! Can you make it to level 1300?"


function SurvivalA3GameRev:new()
	SurvivalA3GameRev.super:new()
	self.grade = 0
	self.garbage = 0
	self.clear = false
...
function SurvivalA3GameRev:initialize(ruleset)
...
function SurvivalA3GameRev:getARE()
		if self.level < 300 then return 12
	else return 6 end
end

function SurvivalA3GameRev:getLineARE()
...
function SurvivalA3GameRev:getDasLimit()
		if self.level < 100 then return 9
	elseif self.level < 500 then return 7
	else return 5 end
end

function SurvivalA3GameRev:getLineClearDelay()
	if self.level < 1300 then return self:getLineARE() - 2
	else return 6 end
end

function SurvivalA3GameRev:getLockDelay()
...
function SurvivalA3GameRev:getGravity()
	return 20
end

function SurvivalA3GameRev:getGarbageLimit()
...
function SurvivalA3GameRev:getSkin()
	return self.level >= 1000 and "bone" or "2tie"
end

function SurvivalA3GameRev:hitTorikan(old_level, new_level)
...
function SurvivalA3GameRev:advanceOneFrame()
	if self.clear then
...
function SurvivalA3GameRev:onPieceEnter()
...
function SurvivalA3GameRev:onLineClear(cleared_row_count)
...
function SurvivalA3GameRev:onPieceLock(piece, cleared_row_count)
	self.super:onPieceLock()
	if cleared_row_count == 0 then self:advanceBottomRow(1) end
end

function SurvivalA3GameRev:updateScore(level, drop_bonus, cleared_lines)
	if not self.clear then
...
function SurvivalA3GameRev:updateSectionTimes(old_level, new_level)
...
function SurvivalA3GameRev:advanceBottomRow(dx)
...
function SurvivalA3GameRev:drawGrid()
	self.grid:draw()
end
...
function SurvivalA3GameRev:drawScoringInfo()
	SurvivalA3GameRev.super.drawScoringInfo(self)
...
function SurvivalA3GameRev:getBackground()
	return math.floor(self.level / 100)
end

function SurvivalA3GameRev:getHighscoreData()
	return {
		level = self.level,
		frames = self.frames,
		grade = self.grade,
