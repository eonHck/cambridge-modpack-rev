...
local MarathonA2GameRev = GameMode:extend()

MarathonA2GameRev.name = "Marathon A2 Rev"
MarathonA2GameRev.hash = "MarathonA2Rev"
MarathonA2GameRev.tagline = "The points don't matter! Can you reach the invisible roll?"




function MarathonA2GameRev:new()
	MarathonA2GameRev.super:new()
...
function MarathonA2GameRev:getARE()
		if self.level < 700 then return 27
	elseif self.level < 800 then return 18
	else return 14 end
end

function MarathonA2GameRev:getLineARE()
...
function MarathonA2GameRev:getDasLimit()
		if self.level < 500 then return 15
	elseif self.level < 900 then return 9
	else return 7 end
end

function MarathonA2GameRev:getLineClearDelay()
...
function MarathonA2GameRev:getLockDelay()
		if self.level < 900 then return 30
	else return 17 end
end

function MarathonA2GameRev:getGravity()
...
function MarathonA2GameRev:advanceOneFrame()
	if self.clear then
...
function MarathonA2GameRev:onPieceEnter()
	if (self.level % 100 ~= 99 and self.level ~= 998) and not self.clear and self.frames ~= 0 then
		self.level = self.level + 1
	end
end

function MarathonA2GameRev:updateScore(level, drop_bonus, cleared_lines)
...
function MarathonA2GameRev:onLineClear(cleared_row_count)
...
function MarathonA2GameRev:updateSectionTimes(old_level, new_level)
...
function MarathonA2GameRev:whilePieceActive()
...
function MarathonA2GameRev:updateGrade(cleared_lines)
...
function MarathonA2GameRev:qualifiesForMRoll()
...
function MarathonA2GameRev:getLetterGrade()
...
MarathonA2GameRev.rollOpacityFunction = function(age)
	if age < 240 then return 1
	elseif age > 300 then return 0
	else return 1 - (age - 240) / 60 end
end

MarathonA2GameRev.mRollOpacityFunction = function(age)
	if age > 4 then return 0
	else return 1 - age / 4 end
end

function MarathonA2GameRev:drawGrid()
...
function MarathonA2GameRev:drawScoringInfo()
...
function MarathonA2GameRev:getHighscoreData()
	return {
...
function MarathonA2GameRev:getSectionEndLevel()
	if self.level >= 900 then return 999
	else return math.floor(self.level / 100 + 1) * 100 end
end

function MarathonA2GameRev:getBackground()
	return math.floor(self.level / 100)
end

return MarathonA2GameRev
