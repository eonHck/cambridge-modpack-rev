	...
	for y = 5, self.height do
		for x = 1, self.width do
			...
			blocks[self.grid[y][x].skin][self.grid[y][x].colour] then
                local rev_x = self.width - x + 1
                local rev_y = self.height - y + 5
				if self.grid_age[y][x] < 2 then
					love.graphics.setColor(1, 1, 1, 1)
					love.graphics.draw(blocks[self.grid[y][x].skin]["F"], 48+rev_x*16, rev_y*16)
				else
					...
					love.graphics.draw(blocks[self.grid[y][x].skin][self.grid[y][x].colour], 48+rev_x*16, rev_y*16)
				end
				if self.grid[y][x].skin ~= "bone" and self.grid[y][x].colour ~= "X" then
					...
					if y > 5 and self.grid[y-1][x] == empty or self.grid[y-1][x].colour == "X" then
						love.graphics.line(48.0+rev_x*16, 16.5+rev_y*16, 64.0+rev_x*16, 16.5+rev_y*16)
					end
					if y < self.height and self.grid[y+1][x] == empty or
					(y + 1 <= self.height and self.grid[y+1][x].colour == "X") then
						love.graphics.line(48.0+rev_x*16, -0.5+rev_y*16, 64.0+rev_x*16, -0.5+rev_y*16)
					end
					if x > 1 and self.grid[y][x-1] == empty then
						love.graphics.line(64.5+rev_x*16, -0.0+rev_y*16, 64.5+rev_x*16, 16.0+rev_y*16)
					end
					if x < self.width and self.grid[y][x+1] == empty then
						love.graphics.line(47.5+rev_x*16, -0.0+rev_y*16, 47.5+rev_x*16, 16.0+rev_y*16)
					end
				end
			end
		end
