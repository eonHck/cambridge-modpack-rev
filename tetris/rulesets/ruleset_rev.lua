...
local RulesetRev = Object:extend()

RulesetRev.name = ""
RulesetRev.hash = ""

-- Arika-type ruleset defaults
RulesetRev.world = false
RulesetRev.colourscheme = {
	I = "R",
	L = "O",
	J = "B",
	S = "M",
...
RulesetRev.softdrop_lock = true
RulesetRev.harddrop_lock = false

RulesetRev.enable_IRS_wallkicks = false
RulesetRev.are_cancel = false
RulesetRev.are = true
RulesetRev.spawn_above_field = false

RulesetRev.next_sounds = {
		I = "I",
		L = "L",
		J = "J",
		S = "S",
...
function RulesetRev:new(game_mode)
	self.game = game_mode
	local bones
...
function RulesetRev:rotatePiece(inputs, piece, grid, prev_inputs, initial)
	local new_inputs = {}
...
function RulesetRev:attemptRotate(new_inputs, piece, grid, initial)
	local rot_dir = 0
...
function RulesetRev:attemptWallkicks(piece, new_piece, rot_dir, grid)
	-- do nothing in default ruleset
end

function RulesetRev:movePiece(piece, grid, move, instant)
...
function RulesetRev:lockPiece(piece, grid, lock_delay, classic_lock)
...
function RulesetRev:get180RotationValue() return 2 end
function RulesetRev:getDefaultOrientation() return 1 end
function RulesetRev:getDrawOffset(shape, orientation) return { x=0, y=0 } end
function RulesetRev:getAboveFieldOffset(shape, orientation)
	if shape == "I" then
		return 1
	else
		return 2
...
function RulesetRev:initializePiece(
...
function RulesetRev:onPieceCreate(piece) end

function RulesetRev:processPiece(
...
function RulesetRev:canPieceMove(piece, grid) return true end
function RulesetRev:canPieceRotate(piece, grid) return true end
function RulesetRev:onPieceMove(piece) end
function RulesetRev:onPieceRotate(piece) end
function RulesetRev:onPieceDrop(piece) end
